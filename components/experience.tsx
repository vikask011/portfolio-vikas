...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-24 bg-gradient-to-b from-white via-gray-50 to-white relative overflow-hidden">
      <div className="absolute top-0 right-0 w-96 h-96 bg-blue-100 rounded-full blur-3xl opacity-20 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-72 h-72 bg-blue-200 rounded-full blur-3xl opacity-30 pointer-events-none" />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="space-y-8">
          {experiences.map((exp, index) => (
            <div
              key={index}
              className="bg-white rounded-2xl p-8 md:p-10 border border-gray-100 shadow-lg hover:shadow-2xl transition-all duration-300 relative overflow-hidden group"
            >
              ...
              <ul className="space-y-4">
                {exp.description.map((point, idx) => (
                  <li key={idx} className="flex items-start gap-4 group/item">
                    <div className="flex-shrink-0 w-2 h-2 mt-2 bg-blue-600 rounded-full group-hover/item:scale-125 transition-transform" />
