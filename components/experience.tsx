...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-24 bg-gradient-to-b from-white via-gray-50 to-white dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-96 h-96 bg-[#d6e6f7] dark:bg-[#2B5FA6] rounded-full blur-3xl opacity-20 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-72 h-72 bg-[#aecdef] dark:bg-[#2B5FA6] rounded-full blur-3xl opacity-30 dark:opacity-20 pointer-events-none" />
      
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <h2 className="text-5xl font-bold text-gray-900 dark:text-white mb-4 text-center">
          Professional Experience
        </h2>
        <div className="w-20 h-1 bg-[#2B5FA6] mx-auto mb-16 rounded-full" />

        <div className="space-y-8">
          {experiences.map((exp, index) => (
            <div
              key={index}
              className="bg-white dark:bg-gray-800 rounded-2xl p-8 md:p-10 border border-gray-100 dark:border-gray-700 shadow-lg hover:shadow-2xl dark:hover:shadow-[#2B5FA6]/20 transition-all duration-300 relative overflow-hidden group"
            >
              <div className="absolute top-0 left-0 w-1 h-full bg-[#2B5FA6] transform scale-y-0 group-hover:scale-y-100 transition-transform duration-300 origin-top" />
              
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
                <div>
                  <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{exp.position}</h3>
                  <p className="text-xl text-[#2B5FA6] dark:text-[#2B5FA6] font-semibold">{exp.company}</p>
                </div>
                <div className="flex flex-col items-start md:items-end gap-2">
                  <div className="flex items-center gap-2 text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 px-4 py-2 rounded-lg">
                    <Calendar size={18} className="text-[#2B5FA6] dark:text-[#2B5FA6]" />
                    <span className="font-medium">{exp.duration}</span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 px-4 py-2 rounded-lg">
                    <MapPin size={18} className="text-[#2B5FA6] dark:text-[#2B5FA6]" />
              ...
              <ul className="space-y-4">
                {exp.description.map((point, idx) => (
                  <li key={idx} className="flex items-start gap-4 group/item">
                    <div className="flex-shrink-0 w-2 h-2 mt-2 bg-[#2B5FA6] dark:bg-[#2B5FA6] rounded-full group-hover/item:scale-125 transition-transform" />
